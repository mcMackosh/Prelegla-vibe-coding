...
import { ChatMessage, sendDocumentChatMessage } from "@/lib/chat";
...
export default function DocumentChatPanel({
  ...
  const greeting: ChatMessage = {
    role: "assistant",
    content: `Hi! I can help you fill out your ${documentName}. Tell me about the parties involved, and I'll ask for anything else I need.`,
  };
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await sendDocumentChatMessage(documentTypeId, nextMessages);
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-40 flex h-[28rem] w-96 max-w-[calc(100vw-3rem)] flex-col overflow-hidden rounded-xl border border-brand-100 bg-white shadow-xl">
          <div className="flex items-center justify-between border-b border-brand-100 bg-brand-900 px-4 py-3">
            <span className="text-sm font-semibold text-white">Document Assistant</span>
            <button
              type="button"
